...
例えば、自分のPCの処理速度が、ジャッジサーバの0.85倍で、遅い場合は以下のように設定します。

```rust
elapsed_time * 0.85 >= self.time_threshold
```

処理速度の比較ですが、もっと良い方法があるかもしれませんが、私は以下の方法を取りました。ABCでは、以下のように入出力情報を提供しているものがあります。
この中で比較的、入力が多く、実行時間が長いものを選んで、入力情報をダウンロードし、自分の環境での実行時間を上述の`get_time()`関数などを使って測定、比較します。
...
AHCでは、入力の一部も色んな関数で使いまわすことが多いようなので、入力の一部もグローバル変数にして、どこからでも呼び出せるようにしておくと、引数地獄から開放され、実装が楽になる。実装方法は以下参照。
