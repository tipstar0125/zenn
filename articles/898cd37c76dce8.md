...
`f64`は、`NaN`の可能性があるため、`std::cmp::Ord`をトレイトに持たないため、`sort`したり、`BinaryHeap`にpushできなかったり、不都合な場合がある。`NaN`にならないことを自分で保証することを条件に、以下の`NotNan`を使用することができる。
...
デバッグを何回も実施することがあり、入力指示が面倒な場合は、`in`ファイルを予めつくっておき、その中に入力情報を保存し、以下のコマンドにより読み込まれる
